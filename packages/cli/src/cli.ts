...
export async function main(originalArgs: readonly string[]): Promise<void> {
  ...
  const visiblePackages = (await installedPackages()).filter(
    (name) => name.startsWith("@zkkontos/") || name === "axios",
  );
  ...
  if (args.selftest) {
    ...
    init += `
      ...
      import {
        ...
        toHex,
        toUtf8,
      } from "@zkkontos/encoding";
      import { sha512, Bip39, Random } from "@zkkontos/crypto";
      import {
        coins,
        encodeAminoPubkey,
        encodeBech32Pubkey,
      ...
      import { assert, arrayContentEquals, sleep } from "@zkkontos/utils";
