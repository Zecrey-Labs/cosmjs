...
describe("Secp256r1", () => {
  ...
  it("can load private keys", async () => {
    ...
    expect(
      await Secp256r1.makeKeypair(
        fromHex("5b44b93366536ca29097ad0327d7f8f2da914ba5ab912249882aea77c5ea499232"),
      ),
    ).toBeTruthy();
    ...
    await Secp256r1.makeKeypair(fromHex("5b44b93366536ca29097ad0327d7f8f2da914ba5ab912249882aea77c5ea4992"))
      .then(() => fail("promise must be rejected"))
      .catch((error) => expect(error.message).toContain("not a valid secp256r1 private key"));
    await Secp256r1.makeKeypair(fromHex("5b44b93366536ca29097ad0327d7f8f2da914ba5ab912249882aea77c5ea499232aa"))
      .then(() => fail("promise must be rejected"))
      .catch((error) => expect(error.message).toContain("not a valid secp256r1 private key"));
    ...
    await Secp256r1.makeKeypair(fromHex("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"))
      .then(() => fail("promise must be rejected"))
      .catch((error) => expect(error.message).toContain("not a valid secp256r1 private key"));
    await Secp256r1.makeKeypair(fromHex("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141"))
      .then(() => fail("promise must be rejected"))
      .catch((error) => expect(error.message).toContain("not a valid secp256r1 private key"));
  });
  ...
  it("verifies unnormalized pyca/cryptography signatures", async () => {
    ...
    for (const [index, row] of data.entries()) {
      const pubkey = (await Secp256r1.makeKeypair(row.privkey)).pubkey;
      const messageHash = sha256(row.message);
      const isValid = await Secp256r1.verifySignature(
        Secp256r1Signature.fromDer(row.signature),
        messageHash,
        pubkey,
      );
  ...
  it("matches normalized pyca/cryptography signatures", async () => {
    ...
    for (const [index, row] of data.entries()) {
      const keypair = await Secp256r1.makeKeypair(row.privkey);
      const messageHash = sha256(row.message);

      // create signature
      const calculatedSignature = await Secp256r1.createSignature(messageHash, row.privkey);

      // verify calculated signature
      const or1 = await Secp256r1.verifySignature(calculatedSignature, messageHash, keypair.pubkey);
      expect(or1).withContext(`(index ${index})`).toEqual(true);

      // verify original signature
      const ok2 = await Secp256r1.verifySignature(
        Secp256r1Signature.fromDer(row.signature),
        messageHash,
        keypair.pubkey,
      );
