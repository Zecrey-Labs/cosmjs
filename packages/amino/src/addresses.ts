...
import {fromBase64, toBech32} from "@zkkontos/encoding";

import {encodeAminoPubkey} from "./encoding";
import {isEd25519Pubkey, isMultisigThresholdPubkey, isSecp256k1Pubkey, isSecp256r1Pubkey, Pubkey} from "./pubkeys";
...
export function rawSecp256r1PubkeyToRawAddress(pubkeyData: Uint8Array): Uint8Array {
  if (pubkeyData.length !== 33) {
    throw new Error(`Invalid Secp256r1 pubkey length (compressed): ${pubkeyData.length}`);
  }
  return ripemd160(sha256(pubkeyData));
}
...
export function pubkeyToRawAddress(pubkey: Pubkey): Uint8Array {
  if (isSecp256k1Pubkey(pubkey)) {
    ...
    const pubkeyData = fromBase64(pubkey.value);
    return rawSecp256r1PubkeyToRawAddress(pubkeyData);
